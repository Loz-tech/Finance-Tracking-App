...
class GetMonthlySummaryUseCase @Inject constructor(
    ...
    suspend operator fun invoke(yearMonth: YearMonth): Flow<MonthlySummary> {
        ...
        return transactionRepository.getTransactionsByDateRange(start, end).combine(
            ...
            val colors = listOf(0xFF006874, 0xFF496364, 0xFF634186, 0xFFBA1A1A, 0xFF8B4A00, 0xFF006E28, 0xFF90416A, 0xFF005CBB)
            ...
            val segments = categoryGroups.entries.mapIndexed { i, (cat, txns) ->
                DonutSegment(cat.name, cat.emoji, txns.sumOf { it.amount.toDouble() }.toFloat(), colors[i % colors.size])
            }
